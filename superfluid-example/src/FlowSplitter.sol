...
contract FlowSplitter is CFASuperAppBase {
  ...
  constructor(
    ...
    if (sideReceiverPortion_ <= 0 || sideReceiverPortion_ == 1000)
      revert INVALID_PORTION();
    if (mainReceiver_ == sideReceiver_) revert SAME_RECEIVERS_NOT_ALLOWED();
    if (mainReceiver_ == address(this) || sideReceiver_ == address(this))
      revert NO_SELF_FLOW();

    ACCEPTED_SUPER_TOKEN = ISuperToken(_acceptedToken);
    CREATOR = creator_;
  ...
  function updateSplit(int96 newSideReceiverPortion_) external {
    ...
    int96 totalOutflowRate = ACCEPTED_SUPER_TOKEN.getFlowRate(
      address(this),
      MAIN_RECEIVER
  ...
  function onFlowUpdated(
    ...
    int96 inflowChange = superToken_.getFlowRate(sender_, address(this)) -
      previousFlowRate_;
  ...
  function onFlowDeleted(
    ...
    int96 remainingInflow = (ACCEPTED_SUPER_TOKEN.getFlowRate(
      address(this),
      MAIN_RECEIVER
    ) + ACCEPTED_SUPER_TOKEN.getFlowRate(address(this), SIDE_RECEIVER)) -
      previousFlowRate_;
    ...
    if (remainingInflow <= 0) {
      newCtx = superToken_.deleteFlowWithCtx(
        address(this),
        MAIN_RECEIVER,
        newCtx
      );
